...
//TODO: Using PubSub - remove asyncmqtt options later 
...
//===SETUP WIFI and TRANSPORT SPECIFICS===
...
#ifdef USE_TCP
  #define TCP_HOST "192.168.86.112"
  #define TCP_PORT 12333
#endif 

#ifdef USE_UDP
  #define UDP_HOST "192.168.86.112"
  ...
  #define ADC_SERVER_URL "http://192.168.86.112:5003/adc_samples"
  #define I2S_SERVER_URL "http://192.168.86.112:5003/i2s_samples"
#endif
  ...
  //===MQTT Library==============
  //PUBSUBMQTT works best; others will be deleted later
  //Choose a MQTT library:
  #define PUBSUBMQTT
  //#define ASYNCMQTT //don't use
  
//TODO - the raw sample test is working.  text is read and interpreted by Rhasppy.
//The mic recording to HERMES MQTT is still not working.  Not sure why.
...
#define NUM_BUF 2
#define BUF_LEN 512
#define SAMPLE_SIZE 512 //Rhasspy needs an audiofeed of 512 bytes+header per message (256 samples *2 bytes per value)

#define MAXCHUNKSIZE 2048 //Sets the pubsubclient buffer size as well as an audiochunk buffer size; 
                          //MAXCHUNKSIZE must be biugger than sample size
...
i2s_pin_config_t i2sPins = {
  ...
  bool firstaudiosessionframe = true;
  ...
  //Initialize PUBSUBMQTT Variant======================
    ...
    if(mqttAudioClient.setBufferSize(MAXCHUNKSIZE)){
  ...
  //tell Rhasspy to start a new session for listening for audio to be transcribed to text (turn on ASR)
  void startHermesTransmission(){
    firstaudiosessionframe = true;
    Serial.println("Starting to transmit audio via hermes-mqtt.");
    transmitting = true;
    transmissionTimer = millis();
    randIDgen(sessionID,30);
    String startHeader = String("{\"siteId\": \"default\",\"sessionId\": \"") + String(sessionID) + String("\",\"lang\": null,\"stopOnSilence\": true,\"sendAudioCaptured\": true,\"wakewordId\": null,\"intentFilter\": null}");
  ...
  //This function sends audioSessionFrames - it assumes the ASP module is on
  #ifdef USE8BIT
  ...
  void sendHermesAudioFrameNew(const int16_t *samples,int samples_read){
    ...
    if(firstaudiosessionframe==true){
      for(int i=0;i<20;i++){
        Serial.printf("HERMES FUNC-%d) sample:%d.\r\n",i, samples[i]);
      }
    }
    //Pers notes online:
    ...
    if(printDebug) Serial.println("Steps 1.");
    uint8_t audioChunk[MAXCHUNKSIZE+44]; 
    //int chunkCount;
    String audioFrameTopic;
    
    if(printDebug) Serial.print("2.");
...
//    x=512;
    ...
    if(printDebug||firstaudiosessionframe) {
      //Serial.println("\r\nData size calc:");
    ...
    /*Story of the sound:
    At this point our sound data was sampled as 32bit signed values (in I2SMEMSSampler::read(int16_t *samples, int count)); 
    then bit shifted by 10 to move the most significant bits to lower positions (drops off the 10 LSBs). Then that
    32bit data is stored into a 16bit array (samples). 

      
    
    */
    
    //Copy the header and samples into the audiochunk array for transmitting
    currentPtr = &samples[0];
    memcpy(&audioChunk[0], &headerArrayPtr[0], 44*sizeof(uint8_t));
    //TODO - working on this - changed to "sizeof(uint16_t)""
    memcpy(&audioChunk[44], currentPtr, samples_read*sizeof(uint16_t));
    if(printDebug||firstaudiosessionframe) {
      //printHexArray(&audioChunk[0],12);
      Serial.println("\r\nPrinting first part of wave Header:");
      printHexArray(&audioChunk[44],20);
    }
 ...
 //TESTING - REPLACE THE ABOVE FRAME TOPIC:
    ...
    //done with first audiosessionframe send - set flag to avoid repeated debug messages
    firstaudiosessionframe = false;
  }
  ...
  void sendTestHermesAudioFrameBySamplingRawAudio(int waveChunkSize, bool playBytes = false){
    ...
    //audioChunk[] this is for pulling from rawData[] with size 45294 for 16bit or LEN8BIT for 8bit; 44 is for header
    ...
    Serial.printf("\r\nPreps completed. File length calc: %lu (%04x). Data section length calc: %lu (%04x).\r\n",x+36,x+36,x,x);
    if(use8bit) {totalLength = LEN8BIT;}else{totalLength = 45294;}
...
void adcWriterTask(void *param)
{
  ...
  while (true)
  {
    ...
    //NOTE: all the TCP/UDP/HTTP sender functions write single bytes onto the stream, one by one.  So converting to 
    // (const uint8_t *) matches the .write or sendData functions
...
void i2sMemsWriterTask(void *param)
{
  ...
  while (true)
  {
    ...
    // (const uint8_t *) matches the .write or sendData functions
      ...
      //   for(int i=0;i<40;i++){
      //     char hexCar[2];
      //     sprintf(hexCar, "%02X", (uint8_t) samples[i]);
      //     Serial.print(hexCar);
      //     if((i+1)%4==0){
      //       Serial.println(" ");
      //     }else{
...
void setup()
{
  ...
  //pause and warn users 
  void slow_blink_LED(int numblinks){
    pinMode(ONBOARD_LED,OUTPUT);
    for(int i =0;i<numblinks;i++){
      Serial.println(numblinks-i);
      delay(1000);
    ...
    //pause and warn users 
  void fast_blink_LED(int numblinks){
    pinMode(ONBOARD_LED,OUTPUT);
    for(int i =0;i<numblinks;i++){
      delay(100);
      digitalWrite(ONBOARD_LED,HIGH);
...
void loop()
{
  // nothing to do here - everything is taken care of by tasks
      ...
      if(!completedAudioTransmission && !transmitting){
        Serial.println("\r\n====\r\nStarting transmitting audio.\r\nWill record for 1.5 seconds after the LED blinks 5 times.\r\n======");
        slow_blink_LED(5);
        i2sSampler->countOneTime=0;//reset counter to 0
        startHermesTransmission();
      }else if(transmitting){
        if(now-transmissionTimer > 1500){
          endHermesTransmission();
        }
      }
    #else //RAWSAMPLETEST
...
/*
  void sendHermesAudioFrame(int16_t *samples,int samples_read){
    //Pers notes online:
    //Rhasspy needs an audiofeed of 512 bytes+header per message
    //Some devices, like the Matrix Voice do 512 16 bit read in one mic read
    //That would be 1024 bytes, so two message are needed in that case.
